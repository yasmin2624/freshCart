      ...
      {cartDetails ? (
        cartDetails?.data?.products?.length === 0 ? (
          <div className="flex justify-center py-8 flex-col items-center h-96">
            <h2 className="text-3xl text-gray-600 dark:text-gray-300 my-4">Empty Cart</h2>
            <Link to={"/products"} className="bg-main p-2 rounded-md text-white my-3">
              Shopping Now
            </Link>
          </div>
        ) : (
          <div className="px-4 sm:px-8 md:px-16 lg:px-32 pt-8 bg-[#F8F9FA] dark:bg-gray-900">
            <div className="flex flex-row justify-between items-center gap-3 sm:gap-6 p-3 mb-3">
              <h2 className="text-sm sm:text-lg md:text-xl text-color dark:text-white">
                Total Products: 
              ...
              <table className="w-full text-xs sm:text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  {cartDetails.data.products.map((product) => (
                    <tr key={product.product._id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-4 sm:px-6 py-4">
                        <div className="flex items-center">
                          ...
                          <input
                            className="w-10 sm:w-14 bg-gray-50 dark:bg-gray-700 border text-gray-900 dark:text-white text-xs sm:text-sm rounded-lg px-2.5 py-1"
                      ...
                      <td className="px-4 sm:px-6 py-4">
                        <a onClick={() => deleteProduct(product.product._id)} className="font-medium text-red-600 dark:text-red-400 hover:underline cursor-pointer">
            ...
            <div className="flex justify-center py-6">
              {isLoading ? (
                ...
                <button onClick={async () => await clearCart()} className="bg-main text-white rounded-md p-2 px-8 sm:px-12 text-lg sm:text-2xl hover:text-black dark:hover:text-white">
                  Clear All
                </button>
              )}
            </div>
